...
def validate(
    ...
    for well in wells:
        ...
        if coordinate_kind == "surface_delta":
            ...
            z_relative = sampled_z - sampled_z[config.prefix_points - 1]
            sampled_tvt = float(metadata["anchor_tvt"]) + offset - z_relative
        else:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Load labels only from original F0-F3 and train on the three "
            "development folds other than --fold."
        ),
    )
    ...
    if args.development_only:
        if args.fold_file is not None or args.splits != 5 or args.fold not in range(4):
            raise ValueError(
                "--development-only requires original GroupKFold(5) and "
                "--fold in {0,1,2,3}"
            )
        identifiers = np.asarray(training_well_ids(args.data_root))
        original = np.empty(len(identifiers), dtype=np.int64)
        for fold_number in range(5):
            _, held = fold_indices(identifiers, fold_number, 5)
            original[held] = fold_number
        train_ids = identifiers[
            (original < 4) & (original != args.fold)
        ].tolist()
        valid_ids = identifiers[original == args.fold].tolist()
        with ThreadPoolExecutor(max_workers=args.workers) as pool:
            train_wells = list(
                pool.map(
                    lambda well_id: load_well(well_id, args.data_root),
                    train_ids,
                )
            )
            valid_wells = list(
    ...
    dataset = AlignmentDataset(
        ...
        coordinate_kind=args.coordinate_kind,
        excursion_sampling_factor=args.excursion_sampling_factor,
        excursion_quantile=args.excursion_quantile,
    )
    loader = DataLoader(
        dataset,
    ...
    model = AlignmentUNet(base=args.base, dropout=args.dropout).to(device)
    if args.resume:
    ...
    print(
        ...
        f"coordinate={args.coordinate_kind} "
        f"development_only={args.development_only} "
        f"dropout={args.dropout:.3f} label_smoothing={args.label_smoothing:.3f} "
        f"weight_decay={args.weight_decay:.6f} "
