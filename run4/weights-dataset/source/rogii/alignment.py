...
COORDINATE_KINDS = ("tvt_delta", "surface_delta")
...
def alignment_objective(
    ...
    if coordinate_kind not in COORDINATE_KINDS:
        raise ValueError(f"unknown alignment coordinate: {coordinate_kind}")
    ...
    surface = (
        prediction + z_relative
        if coordinate_kind == "tvt_delta"
        else prediction
    )
...
def make_alignment_example(
    ...
    supported_image = np.broadcast_to(supported[:, None], mismatch.shape)
    surface_z_relative = None
    if coordinate_kind == "surface_delta":
        # Forward synthesis operates in TVT coordinates: it generates a
        # geological surface and then samples the typewell at TVT=U-Z.  Apply
        # the deterministic surface shear only after every TVT-coordinate
        # augmentation has completed.  Applying it before forward synthesis
        # double-counts Z inside `_forward_synthetic_alignment` and lets that
        # function return TVT labels into a surface-coordinate likelihood.
        surface_z_relative = (
            sampled_z - sampled_z[config.prefix_points - 1]
        )
    ...
    if surface_z_relative is not None:
        state_shifts = surface_z_relative / config.state_step
        mismatch = _shift_state_columns(mismatch, state_shifts)
        pseudo_mismatch = _shift_state_columns(
            pseudo_mismatch, state_shifts
        )
        supported_image = _shift_state_columns(
            supported_image.astype(np.float32), state_shifts
        )
        true_offset = true_offset + surface_z_relative
        distance = config.offsets[:, None] - true_offset[None, :]
        labels = np.argmin(np.abs(distance), axis=0).astype(np.int64)
        metadata["coordinate_z_relative"] = (
            surface_z_relative.astype(np.float32)
        )
    distance = config.offsets[:, None] - true_offset[None, :]
    ...
    # Exact target coordinates remain metadata rather than model inputs.  They
    # let auxiliary derivative heads avoid the artificial one-bin jitter that
    # would result from differencing rounded categorical labels.
    metadata["target_offset"] = true_offset.astype(np.float32)
